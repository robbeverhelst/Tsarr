...
function generateTypeExportContent(service: Service, types: string[]): string {
  ...
  return `// biome-ignore-all assist/source/organizeImports: Auto-generated file with many exports
// Auto-generated type exports for ${serviceName}
...
function updateClientFile(service: Service) {
  ...
  if (!existsSync(clientFile)) {
    console.warn(`⚠️  Client file not found: ${clientFile}`);
    return;
  }
  ...
  if (!reExportPattern.test(content)) {
    ...
    console.log(`✅ Updated ${service} client to re-export types`);
  }
}
...
function generateAllTypeExports() {
  console.log('🔧 Generating automated type exports...\n');
  ...
  for (const service of SERVICES) {
    ...
    if (exportedTypes.length === 0) {
      console.warn(`⚠️  No types found for ${service}, skipping...`);
      continue;
    }
    ...
    console.log(`  ✅ Generated ${outputFile} with ${exportedTypes.length} types`);

    // Update client file to re-export types
    updateClientFile(service);
  }

  console.log('\n🎉 Type export generation complete!');
  console.log('\nGenerated exports include:');

  for (const service of SERVICES) {
    const typesFile = join(process.cwd(), `src/generated/${service}/types.gen.ts`);
    const typeCount = extractExportedTypes(typesFile).length;
    console.log(`  • ${service}: ${typeCount} types`);
  }
...
if (import.meta.main) {
  try {
    ...
    console.error('❌ Error generating type exports:', error);
    process.exit(1);
  }
}
