...
/** Every server the test bed provisioned, so the CLI is exercised on both. */
const TARGETS: SmokeTarget[] = [
  {
    label: `stable ${testbedEnv.JELLYFIN_VERSION ?? '?'}`,
    baseUrl: testbedEnv.JELLYFIN_BASE_URL,
    apiKey: testbedEnv.JELLYFIN_API_KEY,
    userId: testbedEnv.JELLYFIN_USER_ID,
  },
  {
    label: `next ${testbedEnv.JELLYFIN_NEXT_VERSION ?? '?'}`,
    baseUrl: testbedEnv.JELLYFIN_NEXT_BASE_URL,
    apiKey: testbedEnv.JELLYFIN_NEXT_API_KEY,
    userId: testbedEnv.JELLYFIN_NEXT_USER_ID,
  },
].filter(target => target.baseUrl && target.apiKey);

// Rebound per target; the checks below run once per server.
...
const label = (name: string) => `[${targetLabel}] ${name}`;
...
for (const target of TARGETS) {
  ...
  env = {
    ...process.env,
    ...testbedEnv,
    TSARR_JELLYFIN_URL: target.baseUrl,
    TSARR_JELLYFIN_API_KEY: target.apiKey,
  };
    ...
    check(
      ...
      stdout => {
        ...
        return data?.IsFavorite === true
          ? null
          : `expected IsFavorite=true, got ${data?.IsFavorite}`;
      }
    );
  ...
  // ---- session remote control ---------------------------------------------
  {
    ...
    results.push({
      name: label('session pause on unknown session fails'),
  ...
  // ---- invalid input handling ---------------------------------------------
  {
    ...
    results.push({
      name: label('unknown item id fails cleanly'),
  ...
  }
  {
    ...
    results.push({
      name: label('invalid --mode is rejected'),
